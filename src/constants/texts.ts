...
export const TEXTS = {
  ...
  // ===== 班次管理（T05 · 自定义班次）=====
  shiftManagerButton: '班次管理',
  shiftManagerTitle: '班次管理',
  shiftManagerNewTitle: '新增班次',
  shiftManagerEditTitle: '编辑班次',
  shiftManagerBuiltinSection: '内置班次',
  shiftManagerBuiltinHint: '系统预设，不可编辑、不可删除',
  shiftManagerCustomSection: '自定义班次',
  shiftManagerCustomHint: '新增的班次会出现在排班表与轮班里，可随时编辑或删除',
  shiftManagerBuiltinTag: '内置',
  shiftManagerAdd: '新增班次',
  shiftManagerEmpty: '还没有自定义班次，点「新增班次」添加第一个',
  shiftManagerUsedCount: (n: number): string => `已被引用 ${n} 次`,
  shiftManagerMaxReached: (n: number): string => `自定义班次已达上限（${n} 个）`,
  shiftManagerDeleteTitle: '删除班次',
  shiftManagerDeleteMessage: '确定删除该自定义班次吗？',
  shiftManagerDeleteDetailUsed: (n: number): string =>
    `有 ${n} 个排班单元格引用了它，删除后这些单元格将清空为无班次（可撤销）。`,
  shiftManagerDeleteDetailUnused: '该班次当前未被任何排班引用。',
  shiftManagerFormName: '班次名称',
  shiftManagerFormNameHint: '1～8 字',
  shiftManagerFormNameInvalid: '名称需要 1～8 个字',
  shiftManagerFormShort: '简写',
  shiftManagerFormShortHint: '1～3 字，显示在表格单元格中',
  shiftManagerFormShortInvalid: '简写需要 1～3 个字',
  shiftManagerFormColor: '颜色',
  shiftManagerFormColorHint: '点击色块或自行取色，文字颜色会自动适配对比度',
  shiftManagerFormIsWork: '计为工作班次',
  shiftManagerFormIsWorkHint: '关闭则视为休息班次，会计入休息天数',
  shiftManagerFormAuto: '允许算法自动分配',
  shiftManagerFormAutoHint: '默认关闭；开启后生成器可主动分配该班次',
  shiftManagerFormStartTime: '开始时间',
  shiftManagerFormEndTime: '结束时间',
  shiftManagerFormTimeHint: '可选，仅作展示，不影响统计',
  shiftManagerFormTimeInvalid: '开始时间须早于结束时间',
} as const;
