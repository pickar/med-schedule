# 医键排班

> 让排班变简单，早点下班

**医键排班**是一款面向医院科室 / 诊所的**可视化医生排班工具**。打开网页即用，无需注册登录，数据保存在你自己的浏览器里。

在线使用：**https://pickar.github.io/med-schedule/**

---

## 网站简介

医键排班把「谁、哪天、上什么班」浓缩进一张清晰的时间表，支持班次配色、医生管理、一键导出图片，并针对手机做了适配。整个网站是纯前端应用，部署在 GitHub Pages 上，**免费、开源、打开即用**。

它不收集任何个人信息——你的排班数据只保存在本地浏览器（localStorage），关掉网页也不会丢，换设备或清缓存才会清除。

---

## 它解决了什么

排班这件事，最怕「看不清、改不动、通知不到」。

| 痛点 | 说明 |
| --- | --- |
| Excel / 纸质表易错 | 手工填表、复制粘贴，改一处要核好几遍，还容易串行漏格。 |
| 临时换班混乱 | 口头或群里说一声就改，事后谁也记不清原来的班是谁的。 |
| 缺乏统一视图 | 想知道「某人这周哪几天班」得翻半天，没有一目了然的全局表。 |
| 通知总漏人 | 排好了却没及时同步到每个人，上班才发现排错了。 |

---

## 功能介绍

| 功能 | 说明 |
| --- | --- |
| 可视化排班表 | 横轴日期、纵轴医生，点格子即可落班次，整月一眼看清。 |
| 班次与配色 | 内置 11 种班次色标（白班 / 夜班 / 休息 / 值班…），颜色区分直观不串。 |
| 班次自定义与管理 | v1.3.0 新增：所有班次（内置 + 自定义）都能改名、换色、删除；自定义班次支持起止时间、是否工作班、是否可自动分配。详见下文。 |
| 医生管理 | 增删医生、分配固定颜色，谁是谁一目了然。 |
| 一键导出 PNG | 把整张排班表导出成图片，发群、打印都方便。 |
| 本地自动保存 | 每次改动自动存入浏览器，关掉再打开数据还在。 |
| 移动端适配 | 手机全屏自适应（含刘海 / Home 指示条安全区），并支持「添加到主屏幕」像原生 App 使用（v1.2.0）。 |
| 轮班（班次序列循环） | v1.1.0 新增，v1.4.0 增强：为一位或多位医生设定班次序列，按周期自动铺满日期；支持「所有医生」一键套用、按「依次错开 / 全部同步」对齐，并可勾选「应用到所有日期」自动铺满整年。详见下文。 |

---

## 使用流程

五步上手，从打开网站到把排班发出去：

1. **打开网站** — 访问 GitHub Pages 链接（无需登录）。
2. **添加医生** — 在「医生」里录入姓名、分配颜色。
3. **排班次** — 回到「排班」，逐格点选班次。
4. **调整换班** — 重选或清空格子修改；支持撤销 / 重做。
5. **导出分享** — 点「导出 PNG」存图，发群或打印。

> 建议第一次排好后点「导出 PNG」存档一份，方便留底和对账。

---

## 轮班功能（v1.1.0 新增）

有医生提出「能不能一键按规律轮班」，于是新增了**班次序列循环（按医生）**能力。

### 它能做什么

为某一位医生定义一段**班次序列**（例如 `白班 → 夜班 → 休息`），再选择一段连续日期区间，系统会按序列周期自动把班次铺进该医生对应日期的格子里。

示例：序列 `[白班, 夜班, 休息]`，起 `8/1`、止 `8/31`，则 `8/1=白`、`8/2=夜`、`8/3=休`、`8/4=白`…… 以此循环。

### 怎么用

1. 点顶部工具栏的 **「轮班」** 按钮（在「导出 PNG」旁边）。
2. 选择医生，在「班次序列」里添加班次（如 白班 / 夜班 / 休息），可上下移动调整顺序。
3. 选择起始、结束日期。
4. 下方**实时预览**每天会排什么班，并标注冲突数量。
5. 点「应用轮班」写入；写错按一次 `Ctrl+Z` 整段回退。

> 手机端：底部切到排班页 → 右上角菜单里也有「轮班」；在医生抽屉里点某人也能「设轮班」并自动预选。

### 关键特性

- **日历锚定**：请假日、被锁定的格子会被跳过，但**仍消耗序列位**，循环节奏不会被打乱（例如 8/3 是请假，则 8/3 跳过、8/4 仍是白班）。
- **冲突处理**：默认跳过已有非锁定班次（不误覆盖），并提供「覆盖」开关按需开启。
- **整段单次撤销**：轮班写入在撤销栈中是**一条**记录，一次回退即可还原整段。
- **零破坏**：领域模型零变更、不碰持久化结构，老数据无缝兼容。

### v1.4.0 增强：批量套用

- **所有医生**：医生下拉新增「所有医生（N 人）」选项，选中后轮班规则一次套用到名册全体，不必逐个点选。
- **多医生对齐**：选「所有医生」且名册多于 1 人时，提供「多医生起始位」切换：
  - **依次错开**（默认）：第 k 位医生从序列第 k 位起步，保证同一天各班次都有人值。
  - **全部同步**：所有人从序列第 0 位起步，同一天班次完全相同。
- **应用到所有日期**：勾选后日期区间自动取当前查看月份 1 号起算的整年（365 天），两个日期控件转为只读，免去每次开弹窗都手填一遍。
- **预览按医生分组**：批量时每人一行摘要，点开才渲染该医生逐日明细，避免长列表卡顿。

---

## 班次自定义与管理（v1.3.0 新增）

有医生希望班次能按自己科室的真实情况来定——于是把**所有班次**都做成了「可管理」的。

### 它能做什么

- **所有班次都能管**：内置的 11 种班次（白班 / 夜班 / 休息 / 值班…）和你的自定义班次，**都能改名、换色、删除**，不再有「内置不能动」的限制。
- **新增自定义班次**：名称（≤8 字）、简写（≤3 字）、颜色（预设色板 + 自由取色）、是否工作班、可选起止时间、是否可自动分配。
- **编辑即时同步**：改了班次名 / 颜色，排班表单元格、图例、选班面板、PNG 导出图会**同步更新**，屏幕所见即导出所得。
- **删除有保护**：删除一个「正在被排班使用」的班次时，会先提示它出现在多少格里，确认后级联把这些格子置空（不会留下悬空引用）。
- **生成器感知**：被删除的班次，自动排班时不会再被分配出来。

### 怎么用

1. 点顶部「更多 ▸ 班次管理」（手机端在右上角菜单里）。
2. 列表里**每个班次**都带 ✎ 编辑、🗑 删除按钮：
   - 内置班次标「内置」灰标，点编辑可改名称 / 颜色；
   - 自定义班次点「＋新增」创建，或点编辑 / 删除维护。
3. 保存后回到排班表，改动立即生效。

> 自定义班次默认不参与「一键自动排班」（除非在表单里打开「可自动分配」），避免把非标准班混进算法公平性统计。

---

## 为什么选它

- **零门槛** — 纯前端，无需登录注册，打开链接就能用。
- **隐私可控** — 数据只存本地浏览器，不经过任何服务器。
- **医疗审美** — 临床青蓝配色，清爽、专业、不刺眼。
- **移动友好** — 手机随时查班、改班，不在电脑前也行。
- **免费开源** — 托管于 GitHub Pages，无费用、代码透明可自查。
- **误改可退** — 支持撤销 / 重做，排错了一键回到上一步。

---

## 技术栈

- React 19 + Vite 8 + TypeScript
- 纯前端，零运行时第三方依赖（除 React 本身）
- 数据持久化：浏览器 localStorage
- 部署：GitHub Pages（GitHub Actions 自动构建）

---

## 本地运行

```bash
# 安装依赖
npm install

# 启动开发服务器（默认 http://localhost:5173）
npm run dev

# 类型检查
npm run typecheck      # 等价 tsc -b

# 生产构建
npm run build          # 输出到 dist/
```

---

## 部署

仓库已配置 GitHub Actions 工作流（`.github/workflows/deploy.yml`）。向 `main` 分支推送即自动执行：拉取代码 → `npm ci` → `npm run build` → 上传 `dist` → 部署到 GitHub Pages。

首次使用需在仓库 **Settings → Pages** 中将 Source 设为 **GitHub Actions**。

---

## 图文版使用说明

仓库内含一份图文并茂的使用说明 [`guide.html`](./guide.html)，用浏览器打开即可查看（含界面示意图与手机端示意）。

---

## 更新日志

- **v1.4.0**（2026-09-01）轮班批量增强
  - 轮班弹窗医生下拉新增「所有医生（N 人）」，一次套用到名册全体。
  - 新增「多医生起始位」切换：依次错开（默认）/ 全部同步。
  - 新增「应用到所有日期」开关：勾选后自动取当前月份 1 号起算整年（365 天），日期控件只读。
  - 预览按医生分组，批量时逐人折叠，避免长列表卡顿。
- **v1.3.0**（2026-08-24）班次自定义与管理
  - 班次管理抽屉：所有班次（内置 11 种 + 自定义）均可编辑、删除。
  - 新增自定义班次：名称、简写、颜色（预设 + 自由取色）、是否工作班、可选起止时间、是否可自动分配。
  - 内置班次种子进统一列表，编辑 / 删除即时同步到表格、图例、选择器与 PNG 导出。
  - 删除被引用班次前提示引用格数，确认后级联置空；生成器不再分配已删除班次。
  - 存储 schema 升至 v3，旧数据平滑迁移。
- **v1.2.0**（2026-08-20）移动端全屏 + 可添加到主屏幕（PWA）
  - 移动端全屏自适应（`100dvh` + 安全区适配），桌面端样式不变。
  - 支持安卓「安装到主屏幕」、iOS Safari「添加到主屏幕」。
- **v1.1.0** 轮班（班次序列循环）
  - 为某位医生设定班次序列，按周期自动铺满一段日期。

---

## 许可证

本项目以开源方式提供，欢迎自用、二次开发与反馈。
